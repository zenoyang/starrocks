...
public class ValidatorTest extends BaseFormatTest {
    ...
    @Test
    public void testUniqueColumnIdNormal() throws RequestException {
        String createTableSQL = "CREATE TABLE if not exists `demo`.`tb_unique_column_normal` (\n" +
        ...
        String tableName = "tb_unique_column_normal";
        TableSchema tableSchema = restClient.getTableSchema(DEFAULT_CATALOG, DB_NAME, tableName);
        for (int i = 0; i < tableSchema.getColumns().size(); i++) {
            Assertions.assertEquals(i, tableSchema.getColumns().get(i).getUniqueId());
        }
        Assertions.assertTrue(Validator.assignUniqColumnId(tableSchema));
        for (int i = 0; i < tableSchema.getColumns().size(); i++) {
            Assertions.assertEquals(i, tableSchema.getColumns().get(i).getUniqueId());
        }
        tableSchema.getColumns().get(2).setUniqueId(-1);
        Assertions.assertFalse(Validator.assignUniqColumnId(tableSchema));
    }

    @Test
    public void testUniqueColumnIdAbnormal() throws RequestException {
        String createTableSQL = "CREATE TABLE if not exists `demo`.`tb_unique_column_abnormal` (\n" +
                "  `rowId` int(11) NULL COMMENT \"\",\n" +
                "  `c_boolean` BOOLEAN NULL COMMENT \"\",\n" +
                "  `c_tinyint` TINYINT NULL COMMENT \"\",\n" +
                "  `c_smallint` SMALLINT NULL COMMENT \"\",\n" +
        ...
        executeSql(createTableSQL);
        String tableName = "tb_unique_column_abnormal";
        TableSchema tableSchema = restClient.getTableSchema(DEFAULT_CATALOG, DB_NAME, tableName);
        for (int i = 0; i < tableSchema.getColumns().size(); i++) {
            Assertions.assertEquals(-1, tableSchema.getColumns().get(i).getUniqueId());
        }
        Assertions.assertTrue(Validator.assignUniqColumnId(tableSchema));
        for (int i = 0; i < tableSchema.getColumns().size(); i++) {
            Assertions.assertEquals(i, tableSchema.getColumns().get(i).getUniqueId());
    ...
    @ParameterizedTest
    @MethodSource("trueOrFalse")
    public void testUniqueColumnIdSchemaChange(String param) throws RequestException {
        String tableName = "tb_unique_column_schema_change";

        String dropTableSQL = "drop table if exists `demo`.`tb_unique_column_schema_change`";
        executeSql(dropTableSQL);
        String createTableSQL = String.format(
                "CREATE TABLE if not exists `demo`.`tb_unique_column_schema_change` (\n" +
                "  `rowId` int(11) NULL COMMENT \"\",\n" +
                "  `c_boolean` BOOLEAN NULL COMMENT \"\",\n" +
                "  `c_tinyint` TINYINT NULL COMMENT \"\",\n" +
                "  `c_smallint` SMALLINT NULL COMMENT \"\",\n" +
                ...
                "DISTRIBUTED BY HASH(rowId) BUCKETS 1\n" +
                "PROPERTIES (\n" +
                "\"fast_schema_evolution\" = \"%s\",\n" +
                "\"replication_num\" = \"1\"\n" +
                ");", param);
        executeSql(createTableSQL);

        String dropColumnTable = "alter table `demo`.`tb_unique_column_schema_change` DROP COLUMN c_bigint";
        executeSql(dropColumnTable);
        Assertions.assertTrue(waitAlterTableColumnFinished(tableName));

        String addColumnTable = "alter table `demo`.`tb_unique_column_schema_change` ADD COLUMN new_col INT" +
                " DEFAULT \"0\" AFTER c_datetime";
