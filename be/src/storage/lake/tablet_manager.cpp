...
TabletManager::TabletManager(std::shared_ptr<LocationProvider> location_provider, UpdateManager* update_mgr, int64_t cache_capacity)
...
StatusOr<TabletMetadataPtr> TabletManager::get_tablet_metadata(std::shared_ptr<FileSystem> fs, const string& path,
    ...
    ASSIGN_OR_RETURN(auto ptr, load_tablet_metadata(fs, path, fill_cache));
    if (fill_cache) {
